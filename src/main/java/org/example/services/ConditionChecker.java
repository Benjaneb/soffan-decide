...
public class ConditionChecker {
    ...
    */
    ...
    public boolean checkCondition13(Point[] points, int apts, int bpts, double radius1, double radius2) {
        int numpoints = points.length;
        boolean withinRad1 = true;
        boolean withinRad2 = true;

        if (numpoints < 5 || radius2 < 0 || Utils.doubleCompare(radius2, 0) == Utils.CompType.EQ){
            return false;
        }
        ...
        for (int i = 0; i + apts + bpts + 2 < numpoints; i++){
            ...
            if (Utils.doubleCompare(radius, radius2) == Utils.CompType.LT || Utils.doubleCompare(radius, radius2) == Utils.CompType.EQ) {
                withinRad2 = false; // found a set of three points that can fit inside circle
            } 
        }

        if (!withinRad1 && !withinRad2){
            return true;
        }
        else{
            return false;
